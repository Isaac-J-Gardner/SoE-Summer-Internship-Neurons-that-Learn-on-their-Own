...
Ported from the authors' MATLAB code (init.m, SAILnet.m, activities.m, showrfs.m).

IMPORTANT CONCEPTUAL NOTE
-------------------------
snnTorch is built around surrogate-gradient / BPTT training. SAILnet does NOT use
gradients at all: it learns with three *local plasticity rules* (Foldiak / Oja).
So snnTorch is used here ONLY for the leaky-integrate-and-fire (LIF) inference
dynamics. All learning is done by hand, under torch.no_grad(). No autograd, no
surrogate gradient, no loss function.

The LIF cell (snn.Leaky) is configured to reproduce activities.m *exactly*:
  - beta = 1 - eta                  (leak; eta = 0.1 integration step)
  - threshold = theta (per neuron)  (adapted by our own rule, not learned by grad)
  - reset_mechanism = "zero"        (reset to resting potential 0 after a spike)
  - reset_delay = False             (reset takes effect within the same step)
This was verified to match the MATLAB loop bit-for-bit.
"""
...
def train(images_path="IMAGES.mat", num_trials=25000, batch_size=100,
    ...
    N = 256
    net = SAILnet(N=N, M=OC * N, p=p, device=device)

    for t in range(1, num_trials + 1):
        # anneal learning rates for the final stretch, per the paper's Methods
        if lr_schedule and t > num_trials - 10000:
            net.alpha, net.beta, net.gamma = 0.1, 0.001, 0.01
        ...
        if t % log_every == 0:
            print(f"trial {t:6d} | mean rate {Y.mean():.3f} (target {p}) | "
