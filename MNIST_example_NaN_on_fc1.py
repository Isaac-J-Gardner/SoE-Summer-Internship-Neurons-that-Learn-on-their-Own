...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        # Per-neuron autoencoder decode — only during training to save memory
        decoded = None
        if self.training:
...
def train(args, model, device, train_loader, optimizer, epoch, recon_scaler):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        reconstruction_target = features.unsqueeze(1).expand_as(decoded)
...
def main():
    ...
    parser.add_argument('--output-csv', type=str, default='sweep_results.csv',
                        help='Path for output CSV file (default: sweep_results.csv)')
