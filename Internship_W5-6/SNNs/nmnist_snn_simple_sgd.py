...
def events_to_spike_train(x, y, p, t, num_steps=NUM_STEPS, delta_t=DELTA_T_US):
    st = np.zeros((num_steps, NUM_INPUTS), dtype=np.float32)
    ...
    step   = np.minimum(t // delta_t, num_steps - 1)
    neuron = y * SENSOR + x
    st[step, neuron] = 1
    return st
...
class NMNISTDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        spike_train = events_to_spike_train(*events)   # (NUM_STEPS, NUM_INPUTS)
