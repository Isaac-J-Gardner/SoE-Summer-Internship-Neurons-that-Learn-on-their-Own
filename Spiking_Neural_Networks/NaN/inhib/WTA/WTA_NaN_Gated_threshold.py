...
class SAE(nn.Module):
    def __init__(self, inhib_strength, theta_plus, theta_decay):
        super().__init__()
...
i = 1
for inhib in inhib_strength:
    for theta_p in theta_plus:
        for theta_d in theta_decay:
            torch.manual_seed(0)
            path = f'Spiking_Neural_Networks/Images/SNN_NaN_WTA/inhib_{inhib}/thetap_{theta_p}/thetad_{theta_d}'
            print(f"Run {i}: inhib = {inhib}, theta plus = {theta_p}, theta decay = {theta_d}")
            i += 1
            if not os.path.exists(path):
                os.makedirs(path)

            net = SAE(inhib_strength = inhib, theta_plus = theta_p, theta_decay = theta_d).to(device)
