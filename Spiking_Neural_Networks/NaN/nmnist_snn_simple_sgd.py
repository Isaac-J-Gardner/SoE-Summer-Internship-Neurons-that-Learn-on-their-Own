...
class SNN(nn.Module):
    def __init__(self, num_inputs, num_hidden, num_outputs, beta, num_steps):
        ...
        self.decoder = nn.Linear(num_hidden, num_inputs)
        self.lifdecoder = snn.Leaky(beta=beta, spike_grad=spike_grad)
    ...
    def forward(self, x):
        ...
        dec_mem = self.lifdecoder.init_leaky()

        spk_rec, mem_rec = [], []
        dec_spk_rec, dec_mem_rec = [], []
        ...
        for step in range(self.num_steps):
            ...
            dec_cur = self.decoder(spk1)
            dec_spk, dek_mem = self.lifdecoder(dec_cur, dek_mem)

            spk_rec.append(spk2)
            mem_rec.append(mem2)
            dec_spk_rec.append(dec_spk)
            dec_mem_rec.append(dek_mem)

        return torch.stack(spk_rec), torch.stack(mem_rec), torch.stack(dec_spk_rec), torch.stack(dec_mem_rec)
