...
class Net(nn.Module):
    def __init__(self):
        ...
        self.conv1_decoder_weights = nn.Parameter(torch.randn(32, 3, 3)*0.01) #decoder weights for conv1, 32 output channels, 3x3 kernel, initialized to small random values.
        self.conv1_decoder_bias = nn.Parameter(torch.zeros(32, 3, 3)) #decoder bias for conv1, 32 output channels, 3x3 kernel, initialized to zeros
    ...
    def forward(self, x):
        ...
        patches = patches.view(x.size(0), 3, 3, 26, 26)
...
def train(args, model, device, train_loader, optimizer, epoch, recon_scaler):
    ...
    for batch_idx, (data, target) in enumerate(train_loader): #default batch size is 64, batch_idx is the id (0 through 63), data is tensor of shape [64, 1, 28, 28], and target is [64]
        ...
        output, decoded, patches = model(data) #model computs the data, outputs 10 log_softmax values
        task_loss = F.nll_loss(output, target) #negative log-likelihood loss, takes the loss associated with the target and negates it
        recon_loss = F.mse_loss(decoded, patches)
        loss = task_loss + recon_scaler * recon_loss #total loss is the sum of the task loss and the reconstruction loss
...
def main():
    ...
    parser.add_argument('--recon-scaler', type=float, default=1.0, metavar='R',
                        help='reconstruction loss scaler (default: 1.0)')
    ...
    for epoch in range(1, args.epochs + 1):
        train(args, model, device, train_loader, optimizer, epoch, args.recon_scaler)
