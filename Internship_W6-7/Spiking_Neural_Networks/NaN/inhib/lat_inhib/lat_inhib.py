...
N_NEURONS = 20
EIG_FLOOR = 1e-12
inhib_scaler = 0.05        # weight on the inhibition term (0.0 recovers your original SAE)
...
class SAE(nn.Module):
    ...
    def forward(self, x):
        ...
        return x, out, spk_rec       # <-- also return encoder spike trains for the inhibition loss
...
def train(network, trainloader, opti, epoch):
    ...
    for batch_idx, (real_img, labels) in enumerate(trainloader):
        ...
        x, x_recon, spk_rec = network(real_img)
...
for title, mat in [('encoder', W), ('decoder', W2), ('decoder_bias', W3)]:
    ...
    for i, ax in enumerate(axes.flat):
        ...
        ax.axis('off')
    plt.tight_layout()
    plt.savefig(f"Spiking_Neural_Networks/NaN/inhib/lat_inhib/images/{title}_{learn_rate:.2e}.png")
    plt.show()